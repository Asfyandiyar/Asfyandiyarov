...
public class FractalExplorer {
   ...
   //обращаясь к экземпляру класса FractalGenerator, мы сможем генерировать фракталы
    ...
    //класс, в котором определяется цвет для частей фрактала
private class FractalWorker extends SwingWorker<Object, Object> {
       ...
       //этот метод обеспечивает выполнение фонового режима
        ...
        //этот метод приходит в действие, когда фоновая задача завершается
        protected void done()
        {
            for(int x = 0 ; x < displaySize ; ++x)
            {
    ...
    //функция, в котором выполняется включение\отключение элементов интерфейса
